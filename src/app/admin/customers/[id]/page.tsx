  ...
  const fetchCustomerDetail = async () => {
    try {
      ...
      if (res.ok) {
        ...
        showToast('Không thể tải chi tiết khách hàng hoặc bạn không có quyền truy cập.', 'error');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Theo dõi hồ sơ thông tin và lịch sử sử dụng dịch vụ của khách hàng.</p>
        </div>
      </div>
          ...
          <Card>
            <CardContent className="p-6 space-y-5">
              ...
              <div className="pt-4 border-t border-border space-y-3.5 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-medium flex items-center gap-1.5">
                    <Calendar className="w-4 h-4 text-slate-400" /> Ngày tạo khách:
                  </span>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card>
            <CardContent className="py-4 flex items-center justify-between flex-wrap gap-4">
              <h3 className="font-bold text-base text-foreground flex items-center gap-2">
                ...
                <span>Danh sách dịch vụ đã đăng ký ({filteredOrders.length})</span>
              </h3>
          ...
          {filteredOrders.length === 0 ? (
            <EmptyState
              ...
              description={searchTerm ? "Không có đơn hàng nào khớp với từ khóa tìm kiếm." : "Khách hàng này chưa mua đơn hàng nào."}
            />
          ) : (
                    ...
                    {filteredOrders.map((o) => {
                      ...
                      return (
                        <tr key={o.id} className="hover:bg-muted/10 transition-colors">
                          ...
                          <td className="px-5 py-4">
                            <div className="font-bold text-foreground">{o.createdByUser.name}</div>
                            <div className="mt-0.5"><RoleBadge role={o.createdByUser.role} /></div>
                          </td>
                          <td className="px-5 py-4 text-center">
